...
**Head-first response seam:** Commit every genuine recipient-bound Head task before starting the initial non-web research query or its potentially slow web search. Check the run generation and Stop state after the complete Head-task loop and before research. The accepted owner question and those Head events remain durable if research later fails, times out or is withheld. Keep content-free provider-step start/completion timestamps, output kind, safe outcome and duration so first-message delay can be diagnosed without logging private text. Verify the order through persisted events and the authenticated conversation API while synthetic web research is deliberately unresolved; fixture evidence is separate from a live provider probe.
...
**Research-failure seam:** For both initial and Critic-driven attempts, preserve provider-contract/tool-trace rejection and cancellation, but record ordinary query/search failures and timeouts as safe unavailable reasons. Unsafe queries are withheld before web access and remain distinguishable from provider outages. A failed later attempt cannot erase a valid earlier source ledger or imply a fresh check. Attach successful initial sources to the first specialist position rather than the earlier Head task; preserve a legacy Head source association without duplication. Exercise Stop/Continue/Retry without replaying confirmed tasks or rounds, a private contact isolated from the web prompt, and more than eight valid sources. Synthetic success does not establish live search behavior on GoDaddy.
...
The owner's explicit request authorizes the current repository's U-09 repair with U-02 encrypted MySQL persistence, U-03 browser presentation, U-05 evidence and U-08 verification seams. It ports the behavior specified in the sister-repository handoff while retaining this target's GoDaddy listener, Google/CSRF boundary, encrypted MySQL store, exact model/reasoning selections and Electric A v8 presentation.

Deliver as one compatible unit: immutable linked correction attempts and active-order closure; readiness-driven dependent scheduling; accepted specialist/reply/role prompt rendering; 64-character role transport and genuine owner-context filtering; full-body/source secret rejection; Critic-summary and structured-source propagation; one Head-mediated checkpointed follow-up search per selected round; provider-native Usage normalization with encrypted MySQL/recovery/deletion; protected Usage aggregation/UI; reconnecting progress reads; coalesced slow Usage refresh; Chromium/Firefox/WebKit fixtures and target-local receipts. Optional lineage, directives and research fields retain old valid ledgers; terminal historical assessments are never rewritten. The older reader compatibility risk remains operational: take a protected pre-upgrade backup and do not downgrade a database containing new linked attempts or Usage records to an incompatible binary.

Acceptance binds QA-R68–QA-R74 plus their referenced existing checks. Repository completion requires the full Node suite, all three browser engines, artifact/path hygiene, final diff inspection and the SDD checker. The optional `NANODUCK_TEST_MYSQL_URL` suite remains a reported limitation when no isolated test database is supplied. No fixture uses production records, real provider credentials or paid calls.

Runtime activation is separate: the named GoDaddy app must pull the committed revision, apply its owned idempotent schema migration and start the new process before Preview/Published can expose these changes. Verify the injected `PORT`, `0.0.0.0` listener, database leadership, `/healthz`, protected Usage endpoint and synthetic or owner-authorized consultation after activation. Do not interrupt active work, migrate another database or treat repository/browser evidence as deployed proof. Formal release readiness remains `not_evaluated`.

## Verification Plan

All 97 formal checks are defined; executions must retain their actual status and seam. Run exact acceptance clauses from the QA checklist and AC-001–AC-010, then collect the additional visual, heuristic and representative-owner evidence. Preserve positive and negative security paths for all 17 security clauses in the Clause coverage table. Dependency maintenance, operational diagnosis, deletion propagation and isolated recovery belong to U-01/U-07/U-08/U-09 as allocated; they are not a separate service or optional checklist.
