...
22 September 2026 · Revision 7 · Authority: current [product-idea.md](product-idea.md) · Working language: English
...
| ID | Observable obligation | Use cases |
|---|---|---|
| FR-05.1 | Settings exposes functioning model and reasoning selectors separately for Head/specialists and Critic, including the optional Critic provider, using only the current supported catalog. GPT-6 Sol (`gpt-6-sol`) is an additional choice in both Codex model selectors with only catalog-supported reasoning efforts; adding it does not change current selections. | UC-005 |
...
| AC-006 — Preferences and limits (UC-005) | Head and Critic model/reasoning selectors genuinely change valid future preferences while an active run retains its tuple. Both Codex selectors offer GPT-6 Sol with a supported effort; choosing it for future work leaves the current defaults and accepted runs intact. Fresh and pre-revision preferences receive the owner-selected Opus 5.5 / Medium Critic once; a later valid owner save survives restart. Invalid combinations cannot silently substitute. Quota, outage, expired app session and selected-provider reauthorization have distinct truthful recovery; an unused provider causes no warning. |
