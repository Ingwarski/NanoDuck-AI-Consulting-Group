...
22 September 2026 · Revision 10 · Authority: current [product-idea.md](product-idea.md) · Working language: English
...
Success path: 1. Owner submits text with optional supported attachments. 2. System confirms accepted work. 3. Head reads all owner messages and corrections, records the deliverables, selects actual permitted roles for the chosen team size and sends distinct verbatim tasks. 4. Each specialist gives an initial position; Critic reviews each against the complete request and directs a grounded correction or useful question, and each specialist replies. 5. After a whole-team round, Head closes or directs another targeted round within the selected ceiling. 6. Every specialist gives Head a final position, Critic checks those positions and every requested output, and Head delivers Consolidated advice covering all outputs and evidence gaps, provisional when unresolved.
...
| FR-02.3 | Every accepted question uses the owner-selected count of 1, 2, 3 or 5 relevant permitted specialists, or a 1–5 count selected by Head in Auto. Head selects the actual distinct role roster for either mode from the permitted pool after reading the complete owner request; the backend validates and persists roster identity/count but does not choose roles by keyword. Head and Critic are excluded from the count. Leadership Consultant and esoteric roles remain excluded. Spiritual Consultant and Psychotherapist retain their specified doctrine and non-clinical/emergency boundaries. | UC-002 |
| FR-02.4 | Each participating consultant and Critic is a separate ephemeral model invocation. Every role receives the complete ordered owner messages and corrections, all confirmed discussion and direct source ledger, its own role and exact Head assignment, and the accepted runtime-instructions snapshot. Text-only providers are told that attached image pixels are unavailable. Head records requested deliverables and sends a distinct complete task to each actual selected specialist; each successful task is saved and delivered verbatim without wrapper, case-anchor, imperative-word or character-count gates or generic fallback. Each specialist gives an initial position to Critic; each receives at least one Critic challenge and replies. After each complete team round, Head closes or directs another useful round within the selected depth ceiling. Each specialist then gives Head a final position; Critic reviews all final positions and requested outputs together; only then does Head address the owner. No specialist-to-specialist or direct Head-answer route is permitted. | UC-002 |
| FR-02.5 | Critic checks the complete owner request, requested outputs, exact assignment, evidence, numbers and contradictions. It directs a specific stop-and-rework when work is circular, irrelevant, fabricated or unsupported, naming the claim or omission, why it fails and the required evidence, calculation or direct answer. It does not invent a defect in sound work. The consultant materially revises, gives a specific evidence-based objection or acknowledges unavailable evidence; Head arbitrates after the whole-team round. | UC-002 |
...
| FR-02.7 | Head labels its final message Consolidated advice and covers every distinct owner-requested output from the reviewed final positions, Critic assessment and direct sources. It identifies any requested item unsupported by available evidence and gives the necessary actions, main risk and revisit condition without an arbitrary three-action or 2,000-character cap. Outcome must not present a Head assignment or a previous question’s conclusion as the current answer. | UC-002 |
...
| FR-05.3 | Settings provides specialist count 1/2/3/5/Auto, discussion-depth ceiling 1/3/5/Auto and incoming-message sound Knock/Chime/Ripple/Off with direct preview. Specialist count excludes Head and Critic. Every selected specialist receives at least one complete Critic challenge and reply. After each whole-team round, Head closes or directs a targeted further round within the ceiling; Auto permits at most 10 rounds per specialist. Every specialist supplies a final position, Critic reviews all of them, and Head concludes only after those messages commit. Unresolved advice remains provisional. Local browser WAV playback needs no notification permission and has a visual equivalent. The 540,000 ms provider budget remains separate. | UC-005 |
...
| NFR-01.3 | The saved discussion depth is a ceiling: every specialist receives at least one complete challenge/reply, Head reviews the whole team before closing or directing a targeted next round, and Auto never exceeds 10 rounds per specialist. Preserve confirmed roster and progress decisions across Stop, Continue, provider failure and restart without duplicate messages. Keep finite HTTP, storage and provider resource ceilings with explicit failure rather than silent shortening, and retain the ten-minute continuation boundary. | UC-003 |
...
| AC-002 — Useful exchange (UC-002) | Head selects actual permitted roles, records requested deliverables and commits distinct complete tasks verbatim. Every specialist receives a concrete Critic challenge and replies. Head closes or directs another useful whole-team round within the depth ceiling; final positions, joint Critic review and one Head conclusion cover every requested output. A sound position receives no invented defect; unsupported claims receive targeted rework. Stop/Retry preserve confirmed work. |
